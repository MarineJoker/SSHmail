		...
		$(function(){
			$('#dg').datagrid({   
			    ...
			    columns:[[    		           
			        ...
			        {field:'open1',title:'有效商品',width:100,  //字段hot
						formatter: function(value,row,index){
							if(value) {
								return "<input type='checkbox' checked='checked' disabled='true'";
							} else {
			        ...
			        {field:'employee1.type',title:'所属商品类别',width:200, //category.type是商品类别
			        	formatter: function(value,row,index){
			        		if(row.employee1 != null && row.employee1.type != null) {
			        			return row.employee1.type; //如果商品类别不为空，返回商品类别
			        		} else {
			        			return "此商品暂时未分类";
			        		}
						}	
